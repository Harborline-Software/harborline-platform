...
//
// This step records unfinished work, but EXPIRED design verdicts fail on every counted module.
...
import {designReviewSummary} from './design-review-status.mjs'
...
function build() {
  ...
  const designReview = designReviewSummary(modules)
  return {
    schemaVersion: 1,
...
if (argv.includes('--json')) {
  ...
  process.stdout.write(`${receipt.status}: ${receipt.designReview.rule} EXPIRED: ${receipt.designReview.expired.join(', ') || 'none'}\n`)
