...
// Generator output -> plain C# Stryker will mutate: drop the auto-generated marker and the .g.cs name (Stryker skips
// both), and return the .razor the file came from, named by its #pragma checksum line.
export function plainRazor(generated) {
  const source = /^﻿?#pragma checksum "([^"]+)"/.exec(generated)?.[1]
  return {source, text: generated.replace(/^\/\/ <auto-generated\/>\r?\n/m, '')}
}
...
function razorRun(target, config, changed, output) {
  ...
  for (const file of readdirSync(generated, {recursive: true}).filter(name => name.endsWith('.g.cs'))) {
    const {source, text} = plainRazor(readFileSync(path.join(generated, file), 'utf8'))
    const destination = path.join(plain, file.replace(/\.g\.cs$/, '.cs'))
    mkdirSync(path.dirname(destination), {recursive: true})
    writeFileSync(destination, text)
    if (source && changed.includes(path.relative(root, source).replaceAll('\\', '/'))) mutate.push(`**/stryker-razor/**/${path.basename(destination)}`)
  }
