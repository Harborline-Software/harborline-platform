...
|---|---|---|
| Stale-key negative test | `obligation: stale key and missing live-target key fail closed`; [tests/adapter-obligations.test.mjs](../../../../projections/typescript/application/hlp.copilot.contracts/tests/adapter-obligations.test.mjs): `obligation 1 (adapter-contract.md:38): Stale-key negative test` | Production receipt/live-key guard; omitted keys also fail in two compile-negative consumers. |
| Adapter cannot be imported from provider-reachable modules | `tooling/tests/pilot-provider-imports.test.mjs`: `obligation: provider-reachable Platform modules do not import the effect adapter`; [tests/adapter-obligations.test.mjs](../../../../projections/typescript/application/hlp.copilot.contracts/tests/adapter-obligations.test.mjs): `obligation 2 (adapter-contract.md:39): Adapter cannot be imported from provider-reachable modules` | TypeScript-symbol inventory of this package's modules, excluding the public app-facing barrel. An aliased import is the positive control. App-owned modules remain unpinned because they are not compiled here. |
| CP cannot execute without a human gesture | `obligation: CP needs a distinct one-use gesture bound to receipt and target`; [tests/adapter-obligations.test.mjs](../../../../projections/typescript/application/hlp.copilot.contracts/tests/adapter-obligations.test.mjs): `obligation 3 (adapter-contract.md:40): CP cannot execute without a human gesture` | Production proof identity, receipt binding, target recheck and replay refusal. The destination Apply-handler wiring remains unpinned; Platform trusts the app to call the mint only after a human gesture. |
| NEVER cannot be dispatched even through a forged request | `obligation: NEVER cannot dispatch through a forged request`; [tests/adapter-obligations.test.mjs](../../../../projections/typescript/application/hlp.copilot.contracts/tests/adapter-obligations.test.mjs): `obligation 4 (adapter-contract.md:41): NEVER cannot be dispatched even through a forged request` | Production classification and receipt membership guard, with a deliberately forged NEVER command at both AP and NEVER tiers. |
| Authorization failure is fail-closed | `obligation: authorization denial is fail-closed in the host contract`; [tests/adapter-obligations.test.mjs](../../../../projections/typescript/application/hlp.copilot.contracts/tests/adapter-obligations.test.mjs): `obligation 5 (adapter-contract.md:42): Authorization failure is fail-closed` | Executable contract specimen in `tests/fixtures/contract-host.mjs`, with zero effect attempts on denial. Real host authorization remains unpinned: Wave 1 supplies no adapter. |
| Effects are atomic or report failure honestly | `obligation: effects commit atomically or report failure honestly in the host contract`; [tests/adapter-obligations.test.mjs](../../../../projections/typescript/application/hlp.copilot.contracts/tests/adapter-obligations.test.mjs): `obligation 6 (adapter-contract.md:43): Effects are atomic or report failure honestly` | Executable contract specimen stages effects and preserves state on failure. Real host transactions and failure mapping remain unpinned for the same reason. |
