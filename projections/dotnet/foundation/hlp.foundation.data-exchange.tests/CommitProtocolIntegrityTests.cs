...
public sealed class CommitProtocolIntegrityTests
{
    ...
    {
        Assert.Throws<ArgumentException>(() => new AcknowledgementPolicy(new HashSet<ExchangeEffectStatus> { status }));
    }

    [Fact]
    public async Task Effect_claim_prevents_duplicate_delivery_and_stale_completion()
    {
        var store = new InMemoryEffectOutcomeStore();
        ...
        await store.CompleteAsync(new(
            ...
            effect,
            firstAttempt,
            new EffectTerminalOutcome(ExchangeEffectStatus.Applied, "records.applied"),
            now));
        ...
        await Assert.ThrowsAsync<ExchangeRunConflictException>(() => store.CompleteAsync(new(
            batch,
            effect,
            secondAttempt,
            new EffectTerminalOutcome(ExchangeEffectStatus.Failed, "records.failed"),
