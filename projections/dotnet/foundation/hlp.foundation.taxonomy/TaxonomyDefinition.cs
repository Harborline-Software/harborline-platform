...
public sealed record TaxonomyNode(string Code, string Display, string Description, string? ParentCode, TaxonomyNodeStatus Status, DateTimeOffset? PublishedAt, DateTimeOffset? TombstonedAt, string? SuccessorCode, string? DeprecationReason, IReadOnlyList<DisplayHistoryEntry> DisplayHistoryEntries);
public sealed record TaxonomyDefinition(string Tenant, TaxonomyDefinitionId DefinitionId, string Version, TaxonomyGovernanceRegime Governance, string Owner, TaxonomyLineage? DerivedFrom, IReadOnlyList<TaxonomyNode> Nodes, int SchemaVersion = 1, TaxonomyDefinitionEnvelope? Envelope = null);
...
public static class TaxonomyDefinitionJson
{
    ...
    {
        public override TaxonomyDefinitionId Read(ref Utf8JsonReader reader, Type type, JsonSerializerOptions options) => reader.TokenType == JsonTokenType.String ? TaxonomyDefinitionId.Parse(reader.GetString()!) : throw new JsonException("A taxonomy definition id must be a string.");
...
public static class TaxonomyDefinitionAdmission
{
    ...
    public static IReadOnlyList<TaxonomyRefusal> Validate(TaxonomyDefinition definition, TaxonomyAdmissionPhase phase, TaxonomyDefinition? previous = null)
    {
        ...
        for (var index = 0; index < definition.Nodes.Count; index++) if (!byCode.TryAdd(definition.Nodes[index].Code, index)) refusals.Add(new("definition.node_code_duplicate", $"/nodes/{index}/code"));
    ...
    private static void ValidateParents(IReadOnlyList<TaxonomyNode> nodes, IReadOnlyDictionary<string, int> byCode, List<TaxonomyRefusal> refusals)
    {
        for (var start = 0; start < nodes.Count; start++)
        {
            ...
            while (true)
            {
                ...
                positions[current] = seen.Count; seen.Add(current); var parentCode = nodes[current].ParentCode;
    ...
    {
        var priorNodes = previous.Nodes.ToDictionary(node => node.Code, StringComparer.Ordinal);
        for (var index = 0; index < definition.Nodes.Count; index++)
        {
            var node = definition.Nodes[index]; if (!priorNodes.TryGetValue(node.Code, out var prior)) continue;
