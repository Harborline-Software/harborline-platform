...
namespace Harborline.Blocks.Reports.Cartridges.TrialBalance;

/// <summary>
/// W#72 PR 2 — Trial Balance cartridge per Stage 02 §4.2. Reads
/// signed account balances via
/// <c>IGeneralLedgerReadModel</c>, enumerates the chart's
/// accounts via the widened
/// <c>IAccountResolver.GetAccountsAsync</c>, and
...
/// only chart-existence + <c>GLAccount.IsActive</c>; it does
/// NOT (cannot) verify tenant scope because <c>ChartOfAccounts</c>
...
public sealed class TrialBalanceCartridge : IReportCartridge<TrialBalanceParameters, TrialBalanceResult>
{
    ...
    public TrialBalanceCartridge(IReportQuerySource source)
    {
