...
namespace Harborline.Blocks.Reports.Cartridges.ProfitAndLossByProperty;
...
/// and Expense accounts, rolling up per <c>JournalEntryLine.PropertyId</c>
/// to produce a property-level income statement.
/// </summary>
/// <remarks>
/// <para>
/// <b>Journal-scan approach.</b> Because <c>IGeneralLedgerReadModel</c>
/// aggregates per-account totals without the property dimension, this
/// cartridge reads the journal snapshot directly via
/// <c>IJournalStore</c> — the same read-only scan used by
/// <c>InMemoryGeneralLedgerReadModel</c>. This is consistent with
    ...
    : IReportCartridge<ProfitAndLossByPropertyParameters, ProfitAndLossByPropertyResult>
{
    ...
    public ProfitAndLossByPropertyCartridge(IReportQuerySource source)
    {
        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var entry in postedEntries)
        {
            ...
            {
                var propKey = line.PropertyId ?? "Unassigned";
