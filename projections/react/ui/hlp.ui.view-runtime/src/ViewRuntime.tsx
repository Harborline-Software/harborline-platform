...
export function ViewRuntime({ plan, rows, accessibleName = 'View results', empty, onRowActivate, onAction }: ViewRuntimeProps) {
  ...
  return <div className="hl-view-runtime" data-definition-source={source} title={source === undefined ? undefined : `Definition source: ${source}`}>
    {plan.bindings.actions?.map(action => <button key={action.id} type="button" onClick={() => onAction?.(action.id)}>{action.label}</button>)}
