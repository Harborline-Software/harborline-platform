...
function columnWidth(candidate: string, fallback: number): number {
...
export function ViewAuthoringEditor({ value, catalogue, onChange }: ViewAuthoringEditorProps) {
  ...
  return <form className="hl-view-authoring" onSubmit={event => event.preventDefault()}>
    ...
    <Section name="Column treatment">{value.columns.map((column, index) => <div key={index}><input aria-label={`Column ${index + 1} width`} type="number" min="1" value={column.width} onChange={event => replaceColumn(index, { ...column, width: columnWidth(event.currentTarget.value, column.width) })} /><input aria-label={`Column ${index + 1} presentation`} value={column.presentation} onChange={event => replaceColumn(index, { ...column, presentation: event.currentTarget.value })} /></div>)}</Section>
