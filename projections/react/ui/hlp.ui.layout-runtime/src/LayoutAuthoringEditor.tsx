...
import type { LayoutAuthoringBlock, LayoutAuthoringEditorProps, LayoutAuthoringDraft, LayoutAuthoringPageRun, LayoutBindingKind } from './LayoutRuntime.types'
...
function BindingPicker({ index, block, catalogue, onChange }: { index: number; block: LayoutAuthoringBlock; catalogue: LayoutAuthoringEditorProps['catalogue']; onChange: (next: LayoutAuthoringBlock) => void }) {
  ...
  return <>
    <select aria-label={`${label} kind`} value={binding?.kind ?? ''} onChange={event => onChange({ ...block, binding: event.currentTarget.value === '' ? undefined : { kind: event.currentTarget.value as LayoutBindingKind, name: '' } })}>
