...
export interface FixtureCase { id: string; input: Record<string, unknown>; expected: Record<string, unknown> }
...
function readFixture(name: string): FixtureDocument {
  return JSON.parse(readFileSync(resolve(process.cwd(), `../../../../conformance/hlp.ui.window/${name}`), 'utf8')) as FixtureDocument
}
...
export const qualityCases = [...(quality.accessibilityCases ?? []), ...(quality.internationalizationCases ?? []), ...(quality.themingCases ?? []), ...(quality.interactionCases ?? [])]
